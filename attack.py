...
import numpy as np 
from utils import gradient_CW,gradient_FGSM, parallel, unparallel_grad, display_image, get_parameters

def PGD_attack_fast (img_0, 
                    ...
                    original_img, truth, 
                    target_index=1, stride=8, alpha=0.0001, display_iter=300, title='',path= './Outputs',
    ...
    while iter_num<300:
        ...
        current_grad = gradient_FGSM(patch_vec_k = parallel_img_k , 
    ...
    while iter_num<300:
        ...
        for i in range(4,img_0.shape[0]-4,stride): #loop starting form zero to center the output 
            for j in range (4,img_0.shape[1]-4,stride): #loop starting form zero to center the output 
                ...
                grad[i-4:i+4,j-4:j+4] += gradient_CW(  patch_vec_k=patch_vec_k, 
